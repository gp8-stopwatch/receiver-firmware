...
extern "C" void HAL_CAN_RxFifo0MsgPendingCallback (CAN_HandleTypeDef *hcan)
{
        ...
        /* Get RX message */
        if (HAL_CAN_GetRxMessage (hcan, CAN_RX_FIFO0, &header, &data[0]) != HAL_OK) {
                /* Reception Error */
                Error_Handler ();
        }

        CanFrame frame{header, data};
...
Can::Can (ICanCallback *callback, uint32_t prescaler, uint32_t sjw, uint32_t bs1, uint32_t bs2) : callback (callback)
{
        ...
        canHandle.Init.SyncJumpWidth = sjw;
        canHandle.Init.TimeSeg1 = bs1;
        canHandle.Init.TimeSeg2 = bs1;
...
{
        auto txHeader = buff.toNative ();
        uint32_t txMailbox{};

        if (HAL_CAN_AddTxMessage (&canHandle, &txHeader, const_cast<uint8_t *> (buff.getData ().data ()), &txMailbox) != HAL_OK) {
                Error_Handler ();
        }
...
void Can::interrupts (bool on, bool errors)
{
        if (on) {
                if (errors) {
                        __HAL_CAN_ENABLE_IT (&canHandle, CAN_IT_ERROR_WARNING);   // Enable Error warning Interrupt
                        __HAL_CAN_ENABLE_IT (&canHandle, CAN_IT_ERROR_PASSIVE);   // Enable Error passive Interrupt
                        __HAL_CAN_ENABLE_IT (&canHandle, CAN_IT_BUSOFF);          // Enable Bus-off Interrupt
                        __HAL_CAN_ENABLE_IT (&canHandle, CAN_IT_LAST_ERROR_CODE); // Enable Last error code Interrupt
                        __HAL_CAN_ENABLE_IT (&canHandle, CAN_IT_ERROR);           // Enable Error Interrupt
                }

                __HAL_CAN_ENABLE_IT (&canHandle, CAN_IT_RX_FIFO0_MSG_PENDING); // Enable FIFO0 interrupt. CAN_IT_FMP1 os also valid value.
        }
        else {
                __HAL_CAN_DISABLE_IT (&canHandle, CAN_IT_ERROR_WARNING);
                __HAL_CAN_DISABLE_IT (&canHandle, CAN_IT_ERROR_PASSIVE);
                __HAL_CAN_DISABLE_IT (&canHandle, CAN_IT_BUSOFF);
                __HAL_CAN_DISABLE_IT (&canHandle, CAN_IT_LAST_ERROR_CODE);
                __HAL_CAN_DISABLE_IT (&canHandle, CAN_IT_ERROR);
